...
import com.citylens.report.adapter.out.persistence.document.CommentDocument;
...
public class CommentController {
    ...
    public ResponseEntity<Map<String, Object>> createComment(
        ...
        if (!reportRepo.existsById(reportId)) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("success", false, "message", "Report không tồn tại"));
        }

        CommentDocument doc = new CommentDocument();
        doc.setReportId(reportId);
        doc.setUserId(request.userId);
        doc.setUserName(request.userName != null ? request.userName : "Người dùng");
    ...
    public ResponseEntity<Map<String, Object>> deleteComment(
        ...
        if (!commentRepo.existsById(commentId)) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("success", false, "message", "Comment không tồn tại"));
        }
        ...
        Map<String, Object> response = new LinkedHashMap<>();
        response.put("success", true);
        response.put("message", "Đã xóa comment");
        return ResponseEntity.ok(response);
    }

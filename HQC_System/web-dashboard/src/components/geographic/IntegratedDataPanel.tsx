...
export default function IntegratedDataPanel({ 
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `hqc_system_${name.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  ...
  const handleExportJSON = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `hqc_system_${name.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
    URL.revokeObjectURL(url);
    setShowExportMenu(false);
  };
